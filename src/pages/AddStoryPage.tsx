...
import { LuArrowLeft, LuPlus, LuTrash, LuX, LuImage, LuInfo, LuSave, LuCopy, LuEye, LuFileText } from 'react-icons/lu';
...
const AddStoryPage = () => {
  ...
  // Check if we have imported PDF data in session storage
  const importedPages = sessionStorage.getItem('pdfImportedPages');
  const importedTitle = sessionStorage.getItem('pdfImportedTitle');

  const [pages, setPages] = useState<Omit<Page, 'id'>[]>(
    importedPages ? JSON.parse(importedPages) : [{ imageUrl: '', textContent: '' }]
  );
  ...
  // Initialize title from imported PDF if available
  useEffect(() => {
    if (importedTitle) {
    ...
    if (importedPages || importedTitle) {
      sessionStorage.removeItem('pdfImportedPages');
      sessionStorage.removeItem('pdfImportedTitle');
    }
  }, []);
