...
const FlipbookViewer: React.FC<FlipbookViewerProps> = ({
  ...
  const isSinglePageView = (pageNumber: number) => {
    return (
      ...
      (pages.length % 2 === 0 && pageNumber === pages.length)
    );
  };
  ...
  return (
    ...
    >
      ...
      <div
        className={`h-[480px] sm:h-[550px] md:h-[600px] w-[300px] sm:w-[800px] md:w-[900px] perspective-element bg-white rounded-2xl drop-shadow-2xl select-none ${
      ...
      >
        ...
        {rightPageIndex >= 0 && (
          ...
          >
            {/* PAGE FRONT (right) */}
            <div
              className="absolute w-full h-full page-content shadow-page rounded-r-2xl overflow-hidden"
              style={{
                backfaceVisibility: 'hidden',
